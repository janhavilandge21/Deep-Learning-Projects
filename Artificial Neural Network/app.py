...
# Set page
st.set_page_config(layout="wide", page_title="Artificial Neural Network")
# --- 1. Data Preprocessing Function (takes DF as input) ---
@st.cache_data
...
def build_model(input_dim):
    # Initialising the NN (Classification Model)
    model = Sequential()

    # Hidden Layers (using the structure from your notebook)
...
# Epochs selection
epochs = st.sidebar.slider("Select Number of Epochs:", 50, 500, 200, step=50)
...
# Use st.file_uploader to reliably get the file data from the user
...
if uploaded_file is not None:
    try:
        ...
        uploaded_file = None # Reset state if reading fails
...
if uploaded_file is None:
    ...
    * **Architecture:** 16 -> 8 (Dropout 0.25) -> 4 (Dropout 0.5) -> 1 (Sigmoid)
    * **Loss:** Binary Crossentropy
    * **Callbacks:** Early Stopping (patience=20, min\_delta=0.001)
    """)
    ...
    if X_train is None or input_dim == 0:
        ...
        with st.spinner(f"Training model with {optimizer_name} for {epochs} epochs..."):
            ...
            if optimizer_name == 'Adam':
                ...
                optimizer = Adadelta(learning_rate=1.0) # Adadelta uses 1.0 as default
            ...
            # 2. Train the Model and Track Time
